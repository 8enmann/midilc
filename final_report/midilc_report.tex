...
The next step is to initialize all the variables. We will start with the \verb|Note| objects.
...
Next, we initialize the \verb|Sequence| objects. Notice that unlike \verb|Note|, we must call on the new_sequence() function.
...
Now, we are ready to add notes to the \verb|Sequence| objects. Let’s start with 
...
Next, we will start building the main sequence (overall), using a loop 
...
We are almost ready to write the program out to a .csv file. Let us 
...
Congratulations, you have just written your first MIDILC program! 
